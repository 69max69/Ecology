...
def cap(sheet, quater, year):
    ...
    sheet['A6'] = 'за {} квартал {} года'.format(quater, year)
...
def creat_waste_attach(sheet, danger_class, ind, educated_wastes, reclaimed_wastes, transferred_wastes, quater, year):
    ind = ind
    ...
    for waste in wastes:
        ...
        for educated in educated_wastes:
            ...
            month_educated = educated.date_of_educated.month
            year_educated = educated.date_of_educated.year
            quater_educated = (month_educated-1)//3 + 1

            if str(educated.name) == str(waste.name):
                if quater_educated == quater and year_educated == year:
                    educated_sum += educated.quantity
                elif quater_educated < quater and year_educated <= year:
        ...
        for reclaimed in reclaimed_wastes:
            ...
            month_reclaimed = reclaimed.date_of_reclaimed.month
            year_reclaimed = reclaimed.date_of_reclaimed.year
            quater_reclaimed = (month_reclaimed - 1) // 3 + 1

            if str(reclaimed.name) == str(waste.name):
                if quater_reclaimed == quater and year_reclaimed == year:
                    reclaimed_sum += reclaimed.quantity
                elif quater_reclaimed < quater and year_reclaimed <= year:
        ...
        for transferred in transferred_wastes:
            ...
            month_transferred = transferred.date_of_transferred.month
            year_transferred = transferred.date_of_transferred.year
            quater_transferred = (month_transferred - 1) // 3 + 1

            if str(transferred.name) == str(waste.name):
                if quater_transferred == quater and year_transferred == year:
                    transferred_sum += transferred.quantity
                elif quater_transferred < quater and year_transferred <= year:
...
def creat_waste_attach_3(sheet, danger_class, ind, transferred_wastes, quater, year):
...
def get_attachment_quater (data):
    branch = data[0]
    user = data[1]
    quater = int(data[2])
    year = int(data[3])
    ...
    cap(sheet, quater, year)  # создание шапки
    widht_sheet(sheet)
    ...
    for i in range(0, 5):
        ...
        ind_after = creat_waste_attach(sheet, (i + 1), ind, educated_wastes, reclaimed_wastes, transferred_wastes,
                                       quater, year)
    ...
    cap_3(sheet, quater, year)  # создание шапки
    widht_sheet_3(sheet)
    ...
    for i in range(0, 5):
        ...
        ind_after = creat_waste_attach_3(sheet, (i + 1), ind, transferred_wastes, quater, year)
        if ind_after == ind:
    ...
    for ob,pk in zip(object_list, object_pk):
        ...
        for i in range(0, 5):
            ...
            ind_after = creat_waste_attach(sheet, (i+1), ind, educated_wastes, reclaimed_wastes, transferred_wastes, quater, year)
    ...
    filename = f'media/Act/отчет_за_{quater}_квартал_{year}_года_пользователя_{user}_дата_создания_{date1}.xlsx'
    wb.save(filename)
