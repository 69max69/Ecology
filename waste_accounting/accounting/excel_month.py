...
def cap(sheet, month, year):
    ...
    sheet['A6'] = 'за {} месяц {} года'.format(month_list[int(month)-1], year)
...
def creat_waste_attach(sheet, danger_class, ind, educated_wastes, reclaimed_wastes, transferred_wastes, month, year):
    ind = ind
    ...
    for waste in wastes:
        ...
        for educated in educated_wastes:
            ...
            if str(educated.name) == str(waste.name):
                if month_educated == month and year_educated == year:
                    educated_sum += educated.quantity
                elif month_educated < month and year_educated <= year:
        ...
        for reclaimed in reclaimed_wastes:
            ...
            if str(reclaimed.name) == str(waste.name):
                if month_reclaimed == month and year_reclaimed == year:
                    reclaimed_sum += reclaimed.quantity
                elif month_reclaimed < month and year_reclaimed <= year:
        ...
        for transferred in transferred_wastes:
            ...
            if str(transferred.name) == str(waste.name):
                if month_transferred == month and year_transferred == year:
                    transferred_sum += transferred.quantity
                elif month_transferred < month and year_transferred <= year:
...
def creat_waste_attach_3(sheet, danger_class, ind, transferred_wastes, month, year):
...
def creat_titel_page(obj, wb):
    ...
    try:
        ...
        pass
        # branch = NameOfBranch.objects.get(name=obj)
        # sheet['F27']= branch.adress
...
def get_attachment_month (data):
    ...
    cap(sheet, month, year)  # создание шапки
    widht_sheet(sheet)
    ...
    cap_3(sheet, month, year)  # создание шапки
    widht_sheet_3(sheet)
    ...
    for i in range(0, 5):
        ...
        ind_after = creat_waste_attach_3(sheet, (i + 1), ind, transferred_wastes, month, year)
        if ind_after == ind:
    ...
    for ob,pk in zip(object_list, object_pk):
        ...
        for i in range(0, 5):
            ...
            ind_after = creat_waste_attach(sheet, (i+1), ind, educated_wastes, reclaimed_wastes, transferred_wastes, month, year)
    ...
    filename = f'media/Act/отчет_за_{month}_месяц_{year}_года_пользователя_{user}_дата_создания_{date1}.xlsx'
    wb.save(filename)
